...
const hasKeys = new Rule(
  ...
  async (obj, keys) => {
    ...
    // If `obj` is falsy, consider that equivalent to all keys missing.
    if (!obj) {
      throw new Error(`Object missing keys: ${keys.join(', ')}`);
    }
...
const hasOnlyKeys = new Rule(
  ...
  async (obj, keys) => {
    ...
    /*
     * Short-circuit if object is falsy.
     * This satisfies the rule because no extraneous keys exist.
     */
    if (!obj) {
...
const hasOneOfKeys = new Rule(
  ...
  async (obj, keys) => {
    ...
    // Short-circuit if `obj` is falsy.
    // Consider this the same as none of the required keys existing.
    if (!obj) {
...
const hasNoMoreThanOneOfKeys = new Rule(
  ...
  async (obj, keys) => {
    ...
    // Short-circuit if `obj` is falsy, and return a success string.
    // Satisfies rule because no more than one key is specified.
    if (!obj) {
